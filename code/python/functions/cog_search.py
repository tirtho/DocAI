...
def getCogSearchCredential():
  try:
    ...
    if api_key and not api_key.isspace():
      ...
      logging.info("Got Azure Cognitive Search ADMIN API Key from environment variable")
      # Return credential
      return AzureKeyCredential(api_key)
    else:
      # the string is empty
      try:
        logging.info(f"Could not get API key from environment variable COG_SEARCH_ADMIN_KEY. Trying Managed ID")
